...
class General:
    ...
    async def choose(self, *choices):
        """اختار بين الخيارات المتعددة.

       للدلالة على خيارات متعددة، يجب عليك استخدام الاقتباس المزدوج.
        """
    ...
    async def roll(self, ctx, number : int = 100):
        """رمي النرد اختر رقم عشوائي (بين 1 واختيار المستخدم)
    ...
    async def flip(self, ctx, user : discord.Member=None):
        ...
        if user != None:
            ...
            if user.id == self.bot.user.id:
                ...
                msg = "محاولة جيدة. كنت أعتقد أن هذا هو مضحك؟ ماذا عن * هذا * بدلا من ذلك:\n\n"
            ...
            await self.bot.say("* نقلب عملة و ... " + choice(["صورة!*", "كتابة!*"]))
    ...
    async def rps(self, ctx, your_choice : RPSParser):
        ...
        if outcome is True:
            await self.bot.say("{} انت الفائز {}!"
                               "".format(red_choice.value, author.mention))
        elif outcome is False:
            await self.bot.say("{} انت خسران {}!"
                               "".format(red_choice.value, author.mention))
        else:
            await self.bot.say("{} نحن متعادلون {}!"
                               "".format(red_choice.value, author.mention))

    @commands.command(name="8", aliases=["8ball"])
    async def _8ball(self, *, question : str):
        """اسأل الكره رقم 8 سؤال

        يجب أن ينتهي السؤال بعلامة استفهام.
        """
