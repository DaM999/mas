...
class Mod:
    ...
    async def kick(self, ctx, user: discord.Member, *, reason: str = None):
        ...
        if author == user:
            await self.bot.say("لا أستطيع السماح لك بذلك. الايذاء النفسي هو  "
                               "سيء \N{PENSIVE FACE}")
            return
        elif not self.is_allowed_by_hierarchy(server, author, user):
            await self.bot.say("لا أستطيع السماح لك بذلك. انت  "
                               "لست اعلى رتبة من المستخدم المحدد في  "
                               "التسلسل في الرتب .")
            return

        try:
            await self.bot.kick(user)
            logger.info("{}({}) طرد {}({})".format(
            ...
            await self.new_case(server,
                                ...
                                user=user,
                                reason=reason)
            await self.bot.say("تم تنفيذ طلبك , الامر الذي طلبته مني ")
        except discord.errors.Forbidden:
            await self.bot.say("لا يسمح لي بذلك.")
    ...
    async def ban(self, ctx, user: discord.Member, days: str = None, *, reason: str = None):
        """حضر المستخدم مع حذف رسائل المستخدم × ايام

        إذا كانت الأيام ليست رقما، يتم التعامل معها على أنها الكلمة الأولى للسبب.
         الحد الأدنى 0 أيام، والحد الأقصى 7. الافتراضي إلى 0."""
        author = ctx.message.author
        server = author.server
        ...
        try:
            ...
            await self.new_case(server,
                                ...
                                user=user,
                                reason=reason)
            await self.bot.say("تم تنفيذ طلبك. لقد حان وقته .")
        except discord.errors.Forbidden:
            await self.bot.say("لا يسمح لي بذلك.")
        except Exception as e:
            print(e)
    ...
    async def hackban(self, ctx, user_id: int, *, reason: str = None):
        """منع المستخدم من السيرفر بشكل وقائي

         يجب توفير معرف المستخدم (id) .
        إذا كان المستخدم موجود في السيرفر سيكون الحظر العادي
         صدر بدلا من ذلك"""
        user_id = str(user_id)
        author = ctx.message.author
        server = author.server
        ...
        try:
            ...
            await self.new_case(server,
                                ...
                                user=user,
                                reason=reason)
            await self.bot.say("تم تنفيذ طلبك.   لن يتمكن المستخدم من الانضمام إلى هذا "
                               "السيرفر.")

    @commands.command(no_pm=True, pass_context=True)
    @checks.admin_or_permissions(ban_members=True)
    async def softban(self, ctx, user: discord.Member, *, reason: str = None):
        """طرد المستخدم، وحذف قيمة يوم واحد من الرسائل."""
        server = ctx.message.server
        ...
        if can_ban:
            try:
                ...
                await self.bot.say("تم تنفيذ طلبك , لقد فعلت ما يكفي")
            except discord.errors.Forbidden:
    ...
    async def rename(self, ctx, user : discord.Member, *, nickname=""):
        ...
        try:
            ...
            await self.bot.say("تم تنفيذ طلبك . ")
        except discord.Forbidden:
    ...
    async def mute(self, ctx, user : discord.Member, *, reason: str = None):
        """يمكنك اعطاء اسكات للعضو على مستوى الروم او السيرفر
    ...
    async def channel_mute(self, ctx, user : discord.Member, *, reason: str = None):
        ...
        if overwrites.send_messages is False:
            await self.bot.say("هذا المستخدم لا يمكن إرسال رسائل في هذا  "
                               "الروم الكتابي.")
            return
        elif not self.is_allowed_by_hierarchy(server, author, user):
            await self.bot.say("لا أستطيع السماح لك بذلك. انت  "
        ...
        try:
            ...
            await self.new_case(server,
                                ...
                                user=user,
                                reason=reason)
            await self.bot.say("تم تنفيذ طلبك اسكات هذا المستخدم في هذا الروم الكتابي .")
    ...
    async def server_mute(self, ctx, user : discord.Member, *, reason: str = None):
        ...
        await self.new_case(server,
                            ...
                            user=user,
                            reason=reason)
        await self.bot.say("تم تنفيذ طلبك اعطاء هذا العضو اسكات على مستوى السيرفر")

    @commands.group(pass_context=True, no_pm=True, invoke_without_command=True)
    @checks.mod_or_permissions(administrator=True)
    async def unmute(self, ctx, user : discord.Member):
        """يمكنك حذف الاسكات للعضو على مستوى الروم او السيرفر
    ...
    async def channel_unmute(self, ctx, user : discord.Member):
        ...
        try:
            ...
            dataIO.save_json("data/mod/perms_cache.json", self._perms_cache)
            await self.bot.say("تم تنفيذ طلبك فك الاسكات عن هذا المستخدم في هذا الروم الكتابي")
    ...
    async def server_unmute(self, ctx, user : discord.Member):
        ...
        if user.id not in self._perms_cache:
            await self.bot.say("هذا المستخدم لا يبدو ان لديه اسكات . "
        ...
        if user.id in self._perms_cache and not self._perms_cache[user.id]:
            del self._perms_cache[user.id]  # cleanup
        dataIO.save_json("data/mod/perms_cache.json", self._perms_cache)
        await self.bot.say("تم تنفيذ طلبك حذف الاسكات على مستوى السيرفر من هذا العضو  ")
    ...
    async def text(self, ctx, text: str, number: int):
        """لحذف الرسائل × الأخيرة التي تطابق النص المحدد ..
    ...
    async def user(self, ctx, user: discord.Member, number: int):
        ...
        if not has_permissions and not self_delete:
            await self.bot.say("لا يسمح لي بحذف الرسائل.")
            return
    ...
    async def reason(self, ctx, case, *, reason : str=""):
        """يتيح لك تحديد سبب حالات التي تمت من قبل المشرفين - سجل

        يتم تعيين افتراضات لآخر حالة لنفسك، إن وجدت.."""
        author = ctx.message.author
        server = author.server
        try:
            case = int(case)
    ...
    async def ignore(self, ctx):
        """إضافة سيرفر / الرومات إلى قائمة التجاهل"""
        if ctx.invoked_subcommand is None:
            await send_cmd_help(ctx)
    ...
    async def ignore_channel(self, ctx, channel: discord.Channel=None):
        ...
        if not channel:
            if current_ch.id not in self.ignore_list["CHANNELS"]:
                ...
                await self.bot.say("تمت إضافة الروم إلى قائمة التجاهل.")
            else:
                await self.bot.say("الروم بالفعل في قائمة التجاهل.")
        else:
    ...
    async def ignore_server(self, ctx):
        """تجاهل السيرفر الحالي"""
        server = ctx.message.server
        if server.id not in self.ignore_list["SERVERS"]:
            self.ignore_list["SERVERS"].append(server.id)
            dataIO.save_json("data/mod/ignorelist.json", self.ignore_list)
            await self.bot.say("تمت إضافة هذا السيرفر إلى قائمة التجاهل.")
        else:
            await self.bot.say("السيرفر بالفعل في قائمة التجاهل..")

    @commands.group(pass_context=True, no_pm=True)
    @checks.admin_or_permissions(manage_channels=True)
    async def unignore(self, ctx):
        """حذف : السيرفرات / الرومات من قائمة التجاهل"""
        if ctx.invoked_subcommand is None:
            await send_cmd_help(ctx)
            await self.bot.say(self.count_ignored())
    ...
    async def unignore_server(self, ctx):
        """لإزالة السيرفر من قائمة التجاهل"""
        server = ctx.message.server
    ...
    async def _filter(self, ctx):
        """لإضافة / إزالة الكلمات من الفلتر

        استخدام علامةاقتباس مزدوجة : لإضافة / إزالة الجمل
         باستخدام هذا الأمر مع عدم وجود أوامر فرعية سوف ترسل
         قائمة كلمات الخادم التي تمت اضافتها ."""
        if ctx.invoked_subcommand is None:
            await send_cmd_help(ctx)
            server = ctx.message.server
            author = ctx.message.author
            if server.id in self.filter:
                if self.filter[server.id]:
                    ...
                    try:
                        ...
                        await self.bot.say("لا أستطيع إرسال رسائل مباشرة إليك.")

    @_filter.command(name="add", pass_context=True)
    async def filter_add(self, ctx, *words: str):
        """لإضافة كلمات إلى الفلتر

        استخدام علامةاقتباس مزدوجة لإضافة الجمل
        أمثلة:
        filter add كلمه1 كلمه2 كلمه3
        filter add "هذه جملة" """
        if words == ():
            await send_cmd_help(ctx)
            return
        ...
        if added:
            ...
            await self.bot.say("الكلمات المضافة إلى القائمة.")
        else:
            await self.bot.say("الكلمات بالفعل في القائمة.")

    @_filter.command(name="remove", pass_context=True)
    async def filter_remove(self, ctx, *words: str):
        """إزالة الكلمات من الفلتر

        استخدام علامة اقتباس مزدوجة لإزالة الجمل
        أمثلة:
        filter remove كلمة1 كلمة2 كلمة3
        filter remove "هذه الجمله" """
        if words == ():
            await send_cmd_help(ctx)
            return
        server = ctx.message.server
        removed = 0
        if server.id not in self.filter.keys():
            await self.bot.say("لا توجد كلمات تمت اضافتها في هذا السيرفر.")
            return
        for w in words:
        ...
        if removed:
            dataIO.save_json("data/mod/filter.json", self.filter)
            await self.bot.say("الكلمات التي تمت إزالتها من القائمة.")
        else:
            await self.bot.say("لم تكن تلك الكلمات في القائمة.")
    ...
    async def colour(self, ctx, role: discord.Role, value: discord.Colour):
        ...
        استخدم علامات الاقتباس المزدوجة إذا كانت الرتبه تحتوي على مسافات.
        يجيب ان يكون كود اللون مثال :  #ffffff
		هذا موقع يمكنك الاستفادة منه
        \"http://www.w3schools.com/colors/colors_picker.asp\"
        أمثلة:
        !editrole colour "مقدم فعاليات " #ff0000
        !editrole colour  تجربه #ff9900"""
        author = ctx.message.author
        try:
            await self.bot.edit_role(ctx.message.server, role, color=value)
            logger.info("{}({}) تم تنفيذ طلبك تغير لون الرتبة '{}'".format(
                author.name, author.id, role.name))
            await self.bot.say("تم تنفيذ طلبك . ")
        except discord.Forbidden:
            await self.bot.say("احتاج الى  صلاحيات ادارة الرتب.")
        except Exception as e:
            print(e)
            await self.bot.say("هناك خطأ ما..")
    ...
    async def edit_role_name(self, ctx, role: discord.Role, name: str):
        ...
        try:
            ...
            logger.info("{}({}) تم تنفيذ طلبك تغير اسم الرتبه'{}' الى '{}'".format(
    ...
    async def names(self, user : discord.Member):
        """عرض : الأسماء / الألقاب السابقة للمستخدم"""
        server = user.server
