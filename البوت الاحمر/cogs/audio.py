...
class Playlist:
    ...
    def is_author(self, user):
        """يتحقق إذا كان المستخدم هو الذي قام بأنشاء قائمة التشغيل هذه
        يعود صحيح / خطأ"""
        return user.id == self.author

    def can_edit(self, user):
        """يتحقق الآن إذا كان المستخدم هو مشرف أو أعلى بما في ذلك مالك السيرفر
        قوائم التشغيل العامه غير قابلة للتحرير

        ملاحظات المطور :
        ينبغي تعريفها في مكان آخر في وقت لاحق أو أن تكون تلقائية"""
...
class Audio:
    ...
    async def _join_voice_channel(self, channel):
        ...
        try:
            ...
            raise ConnectTimeout("لقد انتهينا من الاتصال بالروم الصوتي،"
                                 " يرجى المحاولة مرة أخرى خلال 10 دقائق.")
    ...
    async def _play(self, sid, url):
        ...
        log.debug('بداء تشغيل على "{}"'.format(server.name))

        if self._valid_playable_url(url) or "[SEARCH:]" in url:
            try:
                song = await self._guarantee_downloaded(server, url)
            except MaximumLength:
                log.warning("لا يمكنني تشغيل عنوان المطلوب أدناه لأنه طويل جدا."
    ...
    async def audiostat_servers(self):
        """عدد السيرفرات قيد التشغيل حاليا."""

        count = self._player_count()

        await self.bot.say("تشغيل الموسيقى حاليا في {} سيرفر .".format(
            count))

    @commands.group(pass_context=True)
    async def cache(self, ctx):
        """أدوات إدارة ذاكرة التخزين المؤقت."""
        if ctx.invoked_subcommand is None:
            await send_cmd_help(ctx)
            return
    ...
    async def cache_dump(self):
        ...
        await self.bot.say("تم تفريغ {: .3f} ميغابايت من الملفات الصوتية.".format(dumped))

    @cache.command(name="minimum")
    async def cache_minimum(self):
        """الحد الأدنى الحالي لحجم ذاكرة التخزين المؤقت، استنادا إلى عدد السيرفرات."""
        await self.bot.say("ستكون ذاكرة التخزين المؤقت على الأقل  {:.3f} ميقابايت".format(
            self._cache_min()))

    @cache.command(name="size")
    async def cache_size(self):
        """الحجم الحالي للذاكرة المؤقتة."""
        await self.bot.say("ذاكرة التخزين المؤقت حاليا في {: .3f} ميغابايت.".format(
            self._cache_size()))

    @commands.group(pass_context=True, hidden=True, no_pm=True)
    @checks.is_owner()
    async def disconnect(self, ctx):
        """قطع الاتصال من الروم الصوتي في السيرفر الحالي."""
        if ctx.invoked_subcommand is None:
            server = ctx.message.server
    ...
    async def disconnect_all(self):
        """قطع الاتصال من جميع الرومات الصوتية."""
    ...
    async def local(self, ctx):
        """أوامر التشغيل للقوائمة الجاهزه . """
        if ctx.invoked_subcommand is None:
            await send_cmd_help(ctx)
    ...
    async def play_local(self, ctx, *, name):
        ...
        if not self.voice_connected(server):
            try:
                ...
                await self.bot.say("يجب ان تدخل الى الروم الصوتي حتى استطيع                    "
                                   "تشغيل اي شيء")
                return
            except UnauthorizedConnect:
                await self.bot.say("ليس لدي الصلاحيات لدخول"
                                   " الروم الصوتي")
                return
            except UnauthorizedSpeak:
                await self.bot.say("ليس لدي صلاحيات التحدث في"
                                   " الروم الصوتي")
                return
    ...
    async def pause(self, ctx):
        """لإيقاف الأغنية الحالية مؤقتا،
		[p] resume للمتابعة."""
        server = ctx.message.server
        if not self.voice_connected(server):
            await self.bot.say("لست في اي روم صوتي في هذا السيرفر")
            return
    ...
    async def _queue(self, ctx, *, url=None):
        """قائمة الاغاني التاليه . وظائف موسعة في `[p] help`
    ...
    async def resume(self, ctx):
        """لاستئناف أغنية أو قائمة تشغيل متوقفة مؤقتا"""
        server = ctx.message.server
        if not self.voice_connected(server):
            await self.bot.say("يجب ان تدخل في الروم الصوتي.")
            return
    ...
    async def skip(self, ctx):
        """تخطي أغنية، وذلك باستخدام التصويت للمجموعة إذا كان الطالب ليس 
		مشرف أو اداري . لا يحسب للمشرف ، والاداري ومالك بوت في
         مجموعة التصويت."""
        msg = ctx.message
        server = ctx.message.server
        if self.is_playing(server):
            ...
            await self.bot.say("لا يمكن تخطي إذا لم يتم التشغيل")
    ...
    async def sing(self, ctx):
        """يجعل البوت يغني واحدة من أغانيه"""
    ...
    async def song(self, ctx):
        """معلومات عن الأغنية الحالية."""
        server = ctx.message.server
        if not self.is_playing(server):
            await self.bot.say(" لم يتم تشغيل على هذا السيرفر .")
            return
    ...
    async def stop(self, ctx):
        ...
        if self.is_playing(server):
            ...
            await self.bot.say("لا يمكن التوقف إذا لم يتم التشغيل")

    @commands.command(name="yt", pass_context=True, no_pm=True)
    async def yt_search(self, ctx, *, search_terms: str):
        """البحث عن مقطع فيديو وتشغيله من اليوتيوب"""
        await self.bot.say("جاري البحث ..")
