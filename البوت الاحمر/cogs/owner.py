...
class Owner:
    ...
    async def _set(self, ctx):
        """التغييرات الأساسية لإعدادات البوت"""
    ...
    async def prefix(self, ctx, *prefixes):
        """لتغيير امر بداية البوت بشكل عام
    ...
    async def serverprefix(self, ctx, *prefixes):
        """لتغيير امر بداية البوت بشكل خاص لهاذا السيرفر
    ...
    async def _server_adminrole(self, ctx, *, role: discord.Role):
        """لعمل رتبة اداري في هذا السيرفر"""
        server = ctx.message.server
    ...
    async def blacklist(self, ctx):
        """أوامر إدارة القائمة السوداء

        لن يتمكن المستخدمون المدرجون في القائمة السوداء من إصدار الأوامر"""
        if ctx.invoked_subcommand is None:
            await self.bot.send_cmd_help(ctx)

    @blacklist.command(name="add")
    async def _blacklist_add(self, user: discord.Member):
        """لإضافة المستخدم إلى القائمة السوداء العامه للبوت"""
    ...
    async def restart(self, silently : bool=False):
        ...
        try:
            if not silently:
                await self.bot.say("جاري اعادة التشغيل ...")
        except:
            pass
    ...
    @checks.is_owner()
    async def join(self):
        """لارسال رابط دعوة الخاص في البوت"""
    ...
    async def servers(self, ctx):
        """قائمة السيرفرات  / بأمكانك اخراج البوت من بعض السيرفرات"""
    ...
    async def info(self):
        ...
        embed.add_field(name="مالك البوت ", value=str(owner))
        embed.add_field(name="اصدار البايثون", value=py_version)
        embed.add_field(name="discord.py اصدار ", value=dpy_version)
        embed.add_field(name="معلومات البوت الاحمر", value=about, inline=False)
        embed.set_footer(text="جلب الفرح منذ 02 يناير 2016 (أكثر"
                         "{} أيام مضت!)".format(days_since))

        try:
            await self.bot.say(embed=embed)
        except discord.HTTPException:
            await self.bot.say("أحتاج إلى صلاحيات وضع الروابط` "
                               "لارسال هذا")

    @commands.command()
    async def uptime(self):
        """لعرض وقت تشغيل البوت"""
